...
const Card = ({ imageSrc1, imageSrc2, title, text }) => {
    return (
...
const Accom = ({ agencyId }) => {
  ...
  useEffect(() => {
    // Fetch data from the API using the agencyId parameter
    fetch(`${variables.API_URL}Accommodation/ByAgency/${agencyId}`)
      .then((response) => response.json())
      .then((data) => {
        // Process the API response and set it to the cardsData state
